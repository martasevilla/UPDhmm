...
calculateEvents <-
    function(largeCollapsedVcf, 
      ...
      split_vcf_raw <-
        ...
        for (name in names_split_vcf_raw) {
            # Access the element using its name
    ...
    if (ncol(df) != 5) {
      stop("Something went wrong while creating dataframes of detected events.")
        ...
        if (!inherits(blocks_state, "list")) {
            stop("Something went wrong during the 
                 transformation into blocks of coordinates.")
        }
      ...
      # Check if `filtered_def_blocks_states` 
      # is a data.frame and with corrected format
        ...
        if (nrow(filtered_def_blocks_states) > 0) {
            blocks_list <- lapply(
                seq_len(nrow(filtered_def_blocks_states)),
                function(i) {
                addOr(
                filtered_def_blocks_states = 
    ...
    # Check if `blocks_list` is in corrected format 
    # and new columns have been added.
        ...
        if (!inherits(block_def, "data.frame")) {
            stop("Something went wrong during final reduction and 
